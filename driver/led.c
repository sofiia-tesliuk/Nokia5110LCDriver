...
static int __init lcd5110_init(void) {
    ...
    initLcdScreen();
    clearLcdScreen();
//    static const unsigned short galka[] = {0x00, 0x8, 0x4, 0x1, 0x4, 0x8, 0x00};
//    int length = sizeof(galka) / sizeof(char);
//    for(int i = 0; i < length; i++){
//        sendByteToLcd(LCD_D, galka[i]);
//    }
    writeStringToLcd("Hi, girls!");
...
ssize_t lcd5110_read(struct file *filp, char *buf,
    ...
    printk( KERN_NOTICE "Device file is read at offset = %i, read bytes count = %u" , (int)*f_pos, (unsigned int) count );
